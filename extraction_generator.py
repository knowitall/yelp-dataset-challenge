...
# Given the result of the parser, extract (noun, adjective) relations
# review and sentence_num are to keep track of where the extraction came from.
  ...
  for stem in stem_to_counts:
    ...
    for attr in stem_to_counts[stem]:
      if stem_to_counts[stem][attr] > max_count:
        ...
        stem_attr_name[stem] = attr
  
  return attr_to_stem, stem_attr_name

def get_extractions_detailed(unstemmed_extractions, attr_to_stem, stem_attr_name):
  extractions_detailed = NestedDict()
  for place in unstemmed_extractions:
    for attr in unstemmed_extractions[place]:
      stem = attr_to_stem[attr.lower()]
      attr_c = stem_attr_name[stem]
      for value in unstemmed_extractions[place][attr]:
        value_c = value.lower()
...
def generate_extractions():
  ...
  print "Stemming attributes"
  attr_to_stem, stem_attr_name = get_stem_info(unstemmed_extractions)
  print "Generating stemmed extractions"
  extractions_detailed = get_extractions_detailed(unstemmed_extractions, 
    attr_to_stem, stem_attr_name)
